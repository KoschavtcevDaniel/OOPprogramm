...
int main(int argc, char* argv[])
{
	...
	//ввод данных
	double x[100];
	if (rank == 0) //если процесс управлени€
	{
	...
	if (world_size > N) //если слишком много процессов дл€ линейного разбиени€
		world_size = N;

	//рассылаем х всем остальным процессам
	MPI_Bcast(x, N, MPI_DOUBLE, 0, MPI_COMM_WORLD);

	//¬џчисление частичной суммы на каждом из процессов
	//на каждом процессе суммируютс€ элеиенты вектора х от ibeg до iend
	//k = N / world_size;
	//ibeg = k * rank;
	//iend = k * (rank + 1);
